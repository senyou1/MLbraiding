...
def test(M,N,L):
    ...
    plt.close()
def plot_energy(tm,tn,m,n,w):
    L=201
    t0=1
    k = np.linspace(0, 2 * np.pi, L, endpoint=True)
    Ek1 = np.sqrt(t0 * (t0 + tm * np.exp(1j * m * k) + tn * np.exp(1j * n * k))).tolist()
    Ek2 = (-np.sqrt(t0 * (t0 + tm * np.exp(1j * m * k) + tn * np.exp(1j * n * k)))).tolist()
    ...
    plt.savefig("w_{}.svg".format(w), bbox_inches='tight', dpi=300, transparent=True)
    plt.show()
if __name__ == "__main__":
    ...
    for i,j in zip(m,n):
        test(i,j,L)
    #参数分别对应tm,tn,m,n,w
    plot_energy(0, 0, 1, 2,0)
    plot_energy(4, 0, 1, 2,1)
    plot_energy(0, -4, 1, 2, 2)
    plot_energy(0, -4, 1, 3, 3)
    plot_energy(0, -4, 3, 4, 4)
    plot_energy(0, -4, 3, 5, 5)
