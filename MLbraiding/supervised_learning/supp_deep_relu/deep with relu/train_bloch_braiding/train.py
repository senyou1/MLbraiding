...
# 合并后的数据生成函数
def generate_data_optimized(t0, tm, tn, m, n, L):
    ...
    # 归一化实部和虚部
    tt1 = E.real / np.sqrt(E.real ** 2 + E.imag ** 2)
    tt2 = E.imag / np.sqrt(E.real ** 2 + E.imag ** 2)
    return np.column_stack((tt1,tt2))[np.newaxis, :], W, tm, tn
...
def create_dataset(ALL, W, n1,n2, T2, T3,L,m,n):
    ...
    plt.savefig('H({},{}).png'.format(m,n),dpi=300)
    plt.show()
    print(ALL.shape)
...
def generate_and_create_dataset(t0, m, n, n1, n2, L):
    T2 = []
    T3 = []
    ALL = []
    W = []
    ...
    # 生成参数列表
    params = [(t0, tm, tn, m, n, L) for tm in np.linspace(-4, 4, n1) for tn in np.linspace(-4, 4, n2)]
    # 并行处理
    results = pool.starmap(generate_data_optimized, params)
    # 关闭进程池
    pool.close()
    pool.join()
    # 提取结果
    for result in results:
        ...
        T3.append(result[3])
    # 创建数据集
    dataset = create_dataset(ALL, W, n1, n2, T2, T3, L, m, n)
    return dataset
def train(L):
    t0 = 1
    m = 1
    ...
    n1 = 300
    n2 = 300
    dataset1 = generate_and_create_dataset(t0, m, n, n1, n2, L)

    m = 1
    n = 2
    dataset2 = generate_and_create_dataset(t0, m, n, n1, n2, L)
